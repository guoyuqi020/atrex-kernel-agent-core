...
Claude uses a fresh session ID with native persistence enabled; it never resumes prior context. Its native main/child JSONLs are retained under `provider/claude-session.raw-jsonl` and `provider/claude-subagents/`, including on timeout or failure. `events.jsonl` contains one latest usage record per response, with `message_id` and `source_path` for joining back to tool calls. Print-stream counters are provisional; repeated updates replace earlier counters. Reconciliation accepts a terminal bill covering either the main session or the whole session tree; accounting always includes each unique child response once. A main-only bill records `claude_terminal_usage_excludes_subagents`. Unreconciled accounting uses the larger known native/terminal count per bucket, marked partial with `claude_response_usage_incomplete_or_unreconciled`, not a verified bill. The raw terminal event remains unchanged. Inspect `session.json.accounting_usage` and `response_usage_complete`; do not sum native/stdout copies or add accounting totals to response usage.

A known Claude accounting gap alone does not trigger exit 126 or skip report completion. Capture failures, unavailable usage, policy violations, nonzero Provider exits, and quota exhaustion remain blocking. Runtime must also support partial accounting reports; otherwise it will reject the handoff. Existing pinned Bundles are not upgraded automatically.
