...
inspected with `list-directions`/`load-direction` and `list-experiments`/`load-experiment`. Any
number of distinct Directions may be proposed, but only one may be `in_progress`; close it before
starting another.
