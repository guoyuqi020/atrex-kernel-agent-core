...
使用 `prompt_root: "repository"`。Optimizer Source 工作副本省略三个初始 State 目录，只有根级
继承版本；其中 Prompts 与 Skills 对 Optimizer 只读，Evolver 的修改由后续新 Session
加载，不改变当前已提交 Prompt。封存 Source 保持完整。

仓库根级 `prompts/`、`skills/`、`tools/` 保存 Runtime State 的初始内容，各自包含 README 索引；
Evolver 只能形成与任务无关的修改，Optimizer 只维护 Tools。没有继承 State 时，Runtime 从固定的 Core Revision
